...
def startup_panel(message):
    try:
        if message.chat.id !=  ADMIN_IDS: return
...
@bot.message_handler(func=lambda m: m.text == "🤖 BOT MODE" and m.chat.id ==  ADMIN_IDS)
...
@bot.message_handler(func=lambda m: m.text == "🔘 BUTTONS" and m.chat.id ==  ADMIN_IDS)
...
@bot.message_handler(func=lambda m: m.text == "📌 AVL BUTTON" and m.chat.id ==  ADMIN_IDS)
...
@bot.message_handler(func=lambda m: m.text == "🔗 GROUP LINK" and m.chat.id ==  ADMIN_IDS)
...
@bot.message_handler(func=lambda m: m.text == "🚪 JOIN RQD" and m.chat.id ==  ADMIN_IDS)
...
@bot.message_handler(func=lambda m: m.text == "💀 DEL BOT" and m.chat.id ==  ADMIN_IDS)
...
def photo_handler(message):
    ...
    try:
        ...
        # User forwarding
        if cid !=  ADMIN_IDS and bot_mode and group_forward_link:
            try:
