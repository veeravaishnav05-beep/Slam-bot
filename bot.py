...
# Forward & join
group_forward_link = None   # @username (bot must be admin there)
join_required_link = None   # @username
...
main_kb.row("☘️Advance Tools")

startup_kb = ReplyKeyboardMarkup(resize_keyboard=True)
startup_kb.row("🫟Wel Msg🫟", "APK/FILE🧭")
...
def send_join_required(uid):
    ...
    # join_required_link is @username, make URL
...
def is_user_member(uid, link):
    """Check if user is member of channel/group (link must be @username)"""
    try:
...
def startup_panel(message):
    try:
        ...
        bot.send_message(message.chat.id, "<b>🚀 𝗦𝗧𝗔𝗥𝗧𝗨𝗣 𝗖𝗢𝗡𝗧𝗥𝗢𝗟</b>\n▰▰▰▰▰▱▱▱▱▱ 50%\n🌟𝗦𝗬𝗦𝗧𝗘𝗠 𝗥𝗘𝗔𝗗𝗬\n👇 CLICK BUTTONS", reply_markup=startup_kb)
    except Exception as e:
...
def custom_panel(message):
    try:
        ...
        bot.send_message(message.chat.id, "<b>📝 𝗖𝗨𝗦𝗧𝗢𝗠 𝗠𝗦𝗚 </b>\n👇 WHAT YOU WANT ?", reply_markup=kb)
    except Exception as e:
...
def demo_panel(message):
    try:
        ...
        demo_temp = []
        state.pop(message.chat.id, None)
        bot.send_message(message.chat.id, "<b>🩵𝗗𝗘𝗠𝗢 𝗦𝗧𝗨𝗗𝗜𝗢🩵</b>\n✨ WHAT YOU WANT ?", reply_markup=demo_admin_kb)
    except Exception as e:
...
def grouplink_handler(message):
    try:
        ...
        if group_forward_link:
            ...
            bot.send_message(message.chat.id, "🔗 Send group/channel **username** (e.g., @mychannel)")
    except Exception as e:
...
def joinrqd_handler(message):
    try:
        ...
        if join_required_link:
            ...
            bot.send_message(message.chat.id, "🔗 Send group/channel **username** for join required")
    except Exception as e:
        print(f"Join RQD handler error: {e}")
    ...
    try:
        ...
        if cid == ADMIN_ID:
            ...
            if ss == "awaiting_group_link":
                link = text.strip()
                # Only accept username (with or without @)
            ...
            if ss == "awaiting_join_link":
                ...
                bot.send_message(cid, "📝 Send join required message")
                return
...
def advance_tools(message):
    try:
        if message.chat.id != ADMIN_ID: return
        bot.send_message(message.chat.id, "<b>🚀 FEATURE COMING SOON</b>")
    except Exception as e:
