...
temp_data = {}          # temporary storage for multi-step flows
...
def send_join_required(uid):
    ...
    buttons.row(InlineKeyboardButton("🚀 𝗝𝗢𝗜𝗡 𝗡𝗢𝗪", url=f"https://t.me/{join_required_link}"))
    try:
...
def is_user_member(uid, link):
    try:
        member = bot.get_chat_member(link, uid)
...
# =========================================================
# STARTUP PANEL & SUB-HANDLERS (keep previous code, will be inserted later)
# I'll put placeholders and then later integrate the full code. For brevity, I'll include full existing working handlers now.
# =========================================================
...
def startup_panel(message):
    ...
    bot.send_message(message.chat.id, "<b>🚀 𝗦𝗧𝗔𝗥𝗧𝗨𝗣 𝗖𝗢𝗡𝗧𝗥𝗢𝗟 𝗖𝗘𝗡𝗧𝗘𝗥</b>\n▰▰▰▰▰▱▱▱▱▱ 50%\n✅ 𝗣𝗥𝗘𝗠𝗜𝗨𝗠 𝗦𝗬𝗦𝗧𝗘𝗠 𝗥𝗘𝗔𝗗𝗬\n👇 CLICK BUTTONS", reply_markup=startup_kb)
...
def welcome_panel(message):
    ...
    kb.row(InlineKeyboardButton("📂 𝗢𝗟𝗗", callback_data="welcome_old"), InlineKeyboardButton("🆕 𝗡𝗘𝗪", callback_data="welcome_new"))
...
def custom_panel(message):
    if message.chat.id != ADMIN_ID: return
    kb = InlineKeyboardMarkup()
    kb.row(InlineKeyboardButton("📂 𝗢𝗟𝗗", callback_data="custom_old"), InlineKeyboardButton("🆕 𝗡𝗘𝗪", callback_data="custom_new"))
...
def setup_panel(message):
    if message.chat.id != ADMIN_ID: return
    kb = InlineKeyboardMarkup()
    kb.row(InlineKeyboardButton("📂 𝗢𝗟𝗗", callback_data="setup_old"), InlineKeyboardButton("🆕 𝗡𝗘𝗪", callback_data="setup_new"))
...
def old_demo(message):
    ...
    if not demo_list:
        ...
        return
    bot.send_message(message.chat.id, f"<b>📂 Saved Demos: {len(demo_list)}</b>")
...
def remove_all_demos(message):
    ...
    if not demo_list:
        ...
        return
    kb = InlineKeyboardMarkup()
    kb.row(InlineKeyboardButton("⚠ CONFIRM", callback_data="demo_remove_all_confirm"), InlineKeyboardButton("❌ CANCEL", callback_data="demo_remove_all_cancel"))
...
def demo_done(message):
    ...
    if not demo_temp:
        ...
        return
    kb = InlineKeyboardMarkup()
    kb.row(InlineKeyboardButton("✅ DONE", callback_data="demo_confirm_done"), InlineKeyboardButton("❌ CANCEL", callback_data="demo_confirm_cancel"))
...
def bot_status(message):
    ...
    uptime = get_uptime()
    dm = message_count
    stats = f"""<b>📡 𝗕𝗢𝗧 𝗦𝗧𝗔𝗧𝗨𝗦</b>
▰▰▰▰▰▰▰▰▰▰
👥 Total Users: {total_users}
👑 Admins: {total_admins}
💎 Paid Users: {total_paid}
📅 Today: {today}
⏰ Uptime: {uptime}
💬 Total DMs: {dm}"""
    if bot_logo:
...
def settings_panel(message):
    if message.chat.id != ADMIN_ID: return
    loading = bot.send_message(message.chat.id, "⚙️ Loading Settings...")
    time.sleep(2)
...
def botmode_handler(message):
    ...
    kb = InlineKeyboardMarkup()
    if bot_mode:
        kb.row(InlineKeyboardButton("🔴 OFF BOT", callback_data="botmode_off"))
    else:
        kb.row(InlineKeyboardButton("🟢 ON BOT", callback_data="botmode_on"))
    bot.send_message(message.chat.id, f"<b>🤖 BOT MODE</b>\nCurrent: {'ON' if bot_mode else 'OFF'}", reply_markup=kb)

@bot.message_handler(func=lambda m: m.text == "🔘 BUTTONS" and m.chat.id == ADMIN_ID)
def buttons_handler(message):
    kb = InlineKeyboardMarkup()
    kb.row(InlineKeyboardButton("🆕 NEW", callback_data="userbtn_new"), InlineKeyboardButton("📂 OLD", callback_data="userbtn_old"))
    bot.send_message(message.chat.id, "<b>🔘 USER BUTTONS</b>\nWHAT YOU WANT ?", reply_markup=kb)
...
def avl_handler(message):
    ...
    for b in user_buttons:
        kb.row(b['name'])
    kb.row("🏠 Main Menu")
    bot.send_message(message.chat.id, "<b>📌 AVAILABLE BUTTONS</b>\nSelect button to set content", reply_markup=kb)
...
def grouplink_handler(message):
    ...
    if group_forward_link:
        kb = InlineKeyboardMarkup()
        kb.row(InlineKeyboardButton("🗑 REMOVE", callback_data="grouplink_remove"))
        bot.send_message(message.chat.id, f"<b>🔗 ALREADY SET</b>\n{group_forward_link}", reply_markup=kb)
    else:
        settings_state[message.chat.id] = "awaiting_group_link"
        bot.send_message(message.chat.id, "🔗 Send group/channel username or invite link")
...
def joinrqd_handler(message):
    ...
    if join_required_link:
        kb = InlineKeyboardMarkup()
        kb.row(InlineKeyboardButton("🆕 NEW", callback_data="joinrqd_new"), InlineKeyboardButton("🗑 REMOVE", callback_data="joinrqd_remove"))
        bot.send_message(message.chat.id, f"<b>🚪 JOIN REQUIRED</b>\nCurrent: {join_required_link}\nWHAT YOU WANT?", reply_markup=kb)
    else:
        settings_state[message.chat.id] = "awaiting_join_link"
        bot.send_message(message.chat.id, "🔗 Send group/channel link for join required")

@bot.message_handler(func=lambda m: m.text == "💀 DEL BOT" and m.chat.id == ADMIN_ID)
def delbot_handler(message):
    kb = InlineKeyboardMarkup()
    kb.row(InlineKeyboardButton("⚠️ CONFIRM", callback_data="delbot_confirm"), InlineKeyboardButton("❌ CANCEL", callback_data="delbot_cancel"))
    bot.send_message(message.chat.id, "<b>💀 DELETE BOT?</b>\nThis will stop the bot completely.", reply_markup=kb)
...
def text_handler(message):
    ...
    if cid == ADMIN_ID:
        ...
        if ss == "awaiting_button_name":
            ...
            kb = InlineKeyboardMarkup()
            kb.row(InlineKeyboardButton("🖼 IMAGES", callback_data="btn_type_photo"),
                   InlineKeyboardButton("🎥 VIDEO", callback_data="btn_type_video"))
            kb.row(InlineKeyboardButton("📱 APPS", callback_data="btn_type_document"),
                   InlineKeyboardButton("📄 FILES", callback_data="btn_type_document"))
        ...
        if ss == "awaiting_group_link":
            ...
            if not (link.startswith("@") or link.startswith("https://t.me/")):
                bot.send_message(cid, "❌ Invalid format. Send username (@channel) or invite link.")
                return
            temp_data[cid] = link
            settings_state[cid] = "awaiting_group_admin"
            kb = InlineKeyboardMarkup()
            kb.row(InlineKeyboardButton("✅ DONE", callback_data="groupadmin_done"),
                   InlineKeyboardButton("❌ CANCEL", callback_data="groupadmin_cancel"))
            bot.send_message(cid, f"🔗 Link: {link}\n<b>PLEASE MAKE BOT ADMIN IN THAT GROUP/CHANNEL THEN PRESS DONE</b>", reply_markup=kb)
            return
        ...
        if ss == "awaiting_join_link":
            ...
            bot.send_message(cid, "📝 Send join required message (shown to users not in channel):")
            return
        # Join message input
        ...
        if ss == "awaiting_avl_select":
            ...
            for i, b in enumerate(user_buttons):
                if b['name'] == text:
                    ...
                    bot.send_message(cid, f"📤 Send new content for button <b>{text}</b> ({b['type']})")
                    return
        ...
        if st == "waiting_custom":
            ...
            kb = InlineKeyboardMarkup()
            kb.row(InlineKeyboardButton("YES ADD MORE", callback_data="custom_add_more"), InlineKeyboardButton("DONE", callback_data="custom_done"))
        ...
        if st == "waiting_broadcast":
            ...
            broadcast_type = "text"
            kb = InlineKeyboardMarkup()
            kb.row(InlineKeyboardButton("YES", callback_data="broadcast_title_yes"), InlineKeyboardButton("NO", callback_data="broadcast_title_no"))
        ...
        if st == "waiting_demo":
            ...
            kb = InlineKeyboardMarkup()
            kb.row(InlineKeyboardButton("YES", callback_data="demo_title_yes"), InlineKeyboardButton("NO", callback_data="demo_title_no"))
    ...
    # USER text: check if matches custom button
    if cid != ADMIN_ID and bot_mode:
        ...
        if text == "🎬 𝗩𝗶𝗲𝘄 𝗗𝗲𝗺𝗼":
            if demo_list:
                bot.send_message(cid, "<b>✨ PREMIUM DEMO SHOWCASE</b>")
                for i, d in enumerate(demo_list):
...
# =========================================================
# PHOTO HANDLER (all photo inputs)
# =========================================================
...
def photo_handler(message):
    ...
    if cid == ADMIN_ID:
        ...
        if st == "waiting_setup":
            ...
            kb = InlineKeyboardMarkup()
            kb.row(InlineKeyboardButton("YES", callback_data="setup_title_yes"), InlineKeyboardButton("NO", callback_data="setup_title_no"))
            bot.send_message(cid, "Add title?", reply_markup=kb)
            return
        if st == "waiting_broadcast":
        ...
        if st == "waiting_demo":
            ...
            return
        # New button content (type already selected)
        if ss == "awaiting_button_content":
            btn_name = temp_data.get(cid, {}).get("button_name")
            btn_type = "photo"
            user_buttons.append({"name":btn_name, "type":btn_type, "file_id":message.photo[-1].file_id})
            settings_state.pop(cid, None)
    ...
    # user photo (ignore if not admin)
    if cid != ADMIN_ID and bot_mode:
        # Could forward to group if group_forward_link set and admin
        if group_forward_link:
            try:
...
def document_handler(message):
    ...
    if cid == ADMIN_ID:
        if st == "waiting_file":
            ...
            kb = InlineKeyboardMarkup()
            kb.row(InlineKeyboardButton("YES", callback_data="file_title_yes"), InlineKeyboardButton("NO", callback_data="file_title_no"))
...
def callback_handler(call):
    ...
    # Settings callbacks
        ...
        tp = data.split("_")[-1]  # photo, video, document
        ...
        try:
            ...
            if member.status in ['administrator', 'creator']:
                ...
                bot.send_message(cid, "❌ Bot not admin. Make admin and press DONE again.", reply_markup=InlineKeyboardMarkup().row(
                    InlineKeyboardButton("✅ RETRY", callback_data="groupadmin_done"), InlineKeyboardButton("❌ CANCEL", callback_data="groupadmin_cancel")))
        except Exception as e:
            bot.send_message(cid, f"❌ Could not check: {e}\nMake sure link is correct and bot is admin. Press RETRY.", reply_markup=InlineKeyboardMarkup().row(
                InlineKeyboardButton("✅ RETRY", callback_data="groupadmin_done"), InlineKeyboardButton("❌ CANCEL", callback_data="groupadmin_cancel")))
        ...
        bot.send_message(cid, "Send new group/channel link")
...
@bot.message_handler(func=lambda m: m.text in ["📡 Bot Status", "🔧 Settings", "⚡ Advance Tools"] and m.chat.id == ADMIN_ID)
def ignore_duplicate(message):
